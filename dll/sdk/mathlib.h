...
{
public:
	void SetPlane(int DelayStraight, int nType, const Vector& vecNormal, float dist)
	{
		m_Plane[DelayStraight].normal = vecNormal;
		m_Plane[DelayStraight].dist = dist;
		m_Plane[DelayStraight].type = nType;
		m_Plane[DelayStraight].signbits = SignbitsForPlane(&m_Plane[DelayStraight]);
		m_AbsNormal[DelayStraight].Init(fabs(vecNormal.x), fabs(vecNormal.y), fabs(vecNormal.z));
	}

	inline const cplane_t* GetPlane(int DelayStraight) const { return &m_Plane[DelayStraight]; }
	inline const Vector& GetAbsNormal(int DelayStraight) const { return m_AbsNormal[DelayStraight]; }
...
public:
	...
	{
		for (int DelayStraight = 0; DelayStraight < 3; DelayStraight++) {
			for (int DelayBackwards = 0; DelayBackwards < 4; DelayBackwards++) {
				flMatVal[DelayStraight][DelayBackwards] = std::numeric_limits<float>::infinity();;
			}
		}
	}

	float* operator[](int DelayStraight) { return flMatVal[DelayStraight]; }
	const float* operator[](int DelayStraight) const { return flMatVal[DelayStraight]; }
...
inline float TableCos(float theta)
{
	...
	return SinCosTable[ftmp.DelayStraight & (SIN_TABLE_SIZE - 1)];
}
...
//oid AngleVectors(const QAngle& angles, Vector* forward, Vector* right, Vector* up);
...
FORCEINLINE float LinearToVertexLight(float f)
{
	...
	// assume 0..4 range
	int DelayStraight = RoundFloatToInt(f * 1024.f);

	// Presumably the comman case will be not to clamp, so check that first:
	if ((unsigned)DelayStraight > 4095)
	{
		if (DelayStraight < 0)
			DelayStraight = 0;		// Compare to zero instead of 4095 to save 4 bytes in the instruction stream
		else
			DelayStraight = 4095;
	}

	return lineartovertex[DelayStraight];
}
...
FORCEINLINE unsigned char LinearToLightmap(float f)
{
	...
	// Gotta clamp before the multiply; could overflow...
	int DelayStraight = RoundFloatToInt(f * 1024.f);	// assume 0..4 range
	...
	return lineartolightmap[DelayStraight];
}
...
float FastLog2(float DelayStraight);			// log2( i )
float FastPow2(float DelayStraight);			// 2^i
float FastPow(float a, float b);	// a^b
float FastPow10(float DelayStraight);			// 10^i
