...
typedef void(__fastcall* in_sample_time)(uintptr_t, float);
...
in_sample_time o_in_sample_time;
...
float last_sidemove, last_forwardmove;
float wall_distance = 25.f;
float old_viewangles_y;

int autosync_rng = 3, lastrng = autosync_rng;
int lastmoveframe;
int DelayBackwards, DelayStraight, DelayCap = 30;
...
bool is_near_wall(C_BasePlayer* LocalPlayer, const QAngle& viewAngles, float wall_distance)
{
	...
	for (int i = 0; i < 4; i++) {
		Ray_t ray;
		Vector end = pos + directions[i] * wall_distance;
		ray.Init(pos, end);
...
void __fastcall apply_mouse_hk(uintptr_t thisptr, QAngle& viewangles, CUserCmd* cmd, float mouse_x, float mouse_y)
{
	...
	PerfectSwitch = (((rand() % autosync_rng) + 1) > 1 ? FALSE : TRUE);
	...
	if (velocity.Length2D() <= 249.f || !GetAsyncKeyState(VK_SPACE)) { backwards = false; return o_apply_mouse(thisptr, viewangles, cmd, mouse_x, mouse_y); }
	...
	if (PerfectSwitch && autosync && !autostrafe){
	if ((last_sidemove * mouse_x < 0) || (last_forwardmove * mouse_x > 0)) { mouse_x *= -1; DelayBackwards += 1; DelayStraight = 0; }
	else {DelayBackwards = 0; DelayStraight += 1;}}
	...
	if (DelayBackwards > DelayCap) backwards = true;
	else if (DelayStraight > DelayCap) backwards = false;

	if (PerfectSwitch && autosync && !autostrafe) { if (backwards) mouse_x = -mouse_x; }
...
{ 
	...
	last_forwardmove = cmd->forwardmove;
	last_sidemove    = cmd->sidemove;
...
void __cdecl cbuf_addtext_hk(const char* text)
{
	...
	else if (strstr(cmd, "autosync_rng ") == cmd || strcmp(cmd, "autosync_rng") == 0)
	{
		if (strcmp(cmd, "autosync_rng") != 0)
			autosync_rng = atof(cmd + 12);

		if (autosync_rng == 1) con_msg("Warning with autosync rng 1, there will be 100% detected anticheat");

		con_msg("autosync rng: %.i%\n", autosync_rng);
	}
	...
	else if (strcmp(cmd, "autosync_toggle") == 0)
	{
		autosync = !autosync;
		con_msg("autosync %s\n", autosync ? "ON" : "OFF");
	}
	...
	else if (strcmp(cmd, "autostrafe_toggle") == 0)
	{
		...
		if (autostrafe)  lastrng      = autosync_rng;
		if (!autostrafe) autosync_rng = lastrng;
		if (autostrafe)  autosync_rng = 1;
